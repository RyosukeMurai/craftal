...
import useCase.account.reponse.AccountRegistrationResponse
...
@Singleton
class RegisterAccount @Inject()(userRepository: UserRepository,
  ...
  def execute(name: String, loginInfo: LoginInfo, password: String, verificationExpiration: FiniteDuration = 5 minutes): Future[AccountRegistrationResponse] = {
    ...
    } yield AccountRegistrationResponse(UserIdentity(userId, Option(user.email), Option(user.name), loginInfo, activated = false), token)
  }
}
