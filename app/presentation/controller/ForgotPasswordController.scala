...
import presentation.form.ForgotPasswordForm
...
class ForgotPasswordController @Inject()(
  ...
  def view: Action[AnyContent] = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    Future.successful(Ok(presentation.view.auth.html.forgotPassword(ForgotPasswordForm.form)))
  }
  ...
  def submit: Action[AnyContent] = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    ForgotPasswordForm.form.bindFromRequest.fold(
      form => Future.successful(BadRequest(presentation.view.auth.html.forgotPassword(form))),
      email => {
          ...
          case Some(user) if user.email.isDefined =>
            authTokenService.create(user.userID).map { authToken =>
              ...
              mailerClient.send(Email(
                ...
                to = Seq(email),
                bodyText = Some(presentation.view.auth.emails.txt.resetPassword(user, url).body),
                bodyHtml = Some(presentation.view.auth.emails.html.resetPassword(user, url).body)
              ))
              result
            }
