...
import presentation.form.ResetPasswordForm
...
class ResetPasswordController @Inject()(
  ...
  def view(token: UUID): Action[AnyContent] = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    authTokenService.validate(token).map {
      case Some(_) => Ok(presentation.view.auth.html.resetPassword(ResetPasswordForm.form, token))
    ...
    authTokenService.validate(token).flatMap {
      case Some(authToken) =>
        ResetPasswordForm.form.bindFromRequest.fold(
          form => Future.successful(BadRequest(presentation.view.auth.html.resetPassword(form, token))),
