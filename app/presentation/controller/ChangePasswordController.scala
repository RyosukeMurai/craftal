...
import application.auth.{DefaultEnv, WithProvider}
...
import presentation.form.ChangePasswordForm
...
class ChangePasswordController @Inject() (
  ...
  def view: Action[AnyContent] = silhouette.SecuredAction(WithProvider[DefaultEnv#A](CredentialsProvider.ID)) {
    implicit request: SecuredRequest[DefaultEnv, AnyContent] =>
      Ok( presentation.view.auth.html.changePassword(ChangePasswordForm.form, request.identity))
  }
  ...
  def submit: Action[AnyContent] = silhouette.SecuredAction(WithProvider[DefaultEnv#A](CredentialsProvider.ID)).async {
    implicit request: SecuredRequest[DefaultEnv, AnyContent] =>
      ChangePasswordForm.form.bindFromRequest.fold(
        form => Future.successful(BadRequest( presentation.view.auth.html.changePassword(form, request.identity))),
        password => {
