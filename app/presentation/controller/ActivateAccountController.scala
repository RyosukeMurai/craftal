      ...
      case Some(user) if !user.activated =>
        authTokenService.create(user.userID).map { authToken =>
          ...
          mailerClient.send(Email(
            ...
            to = Seq(decodedEmail),
            bodyText = Some(presentation.view.auth.emails.txt.activateAccount(user, url).body),
            bodyHtml = Some(presentation.view.auth.emails.html.activateAccount(user, url).body)
          ))
          result
        }
