...
class EventDetailViewPresenter @Inject()(implicit webJarsUtil: WebJarsUtil,
  ...
  def present(values: (Event, List[Photo], List[Prefecture], List[Artist]))
  ...
  def present(event: Event, photos: List[Photo], prefectures: List[Prefecture], artists: List[Artist])
             (implicit request: NavigationContext[AnyContent], messages: Messages): HtmlFormat.Appendable =
    net.craftal.web.view.event.html.detail(
      EventDetailDataMapper.transform(event, photos, prefectures, artists)
    )
}
