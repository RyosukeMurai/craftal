...
class RetryUserActivation @Inject()(authenticationService: AuthenticationService,
    ...
    this.authenticationService.getUser(email).flatMap {
      case Some(x) =>
        for {
          ...
          result <- this.notificationService.notifyRetryUserActivation(email, Option(x.name), token)
        } yield result
