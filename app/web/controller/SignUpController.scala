        ...
        userService.retrieve(loginInfo).flatMap {
          case Some(user) =>
            ...
            mailerClient.send(Email(
              ...
              to = Seq(data.email),
              bodyText = Some(web.view.auth.emails.txt.alreadySignedUp(user, url).body),
              bodyHtml = Some(web.view.auth.emails.html.alreadySignedUp(user, url).body)
            ))
          ...
          case None =>
            ...
            for {
              ...
              mailerClient.send(Email(
                ...
                from = Messages("email.from"),
                to = Seq(data.email),
                bodyText = Some(web.view.auth.emails.txt.signUp(user, url).body),
                bodyHtml = Some(web.view.auth.emails.html.signUp(user, url).body)
              ))
